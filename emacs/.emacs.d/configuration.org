...
*** Paths
Include user binaries. Note: ~/opt/homebrew/bin~ is also set earlier during
package management setup so ~use-package-ensure-system-package~ can find ~brew~.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_QUOTE
Space-separated, order-independent completion matching. Type =conf ruby=
to match both =configure-ruby= and =ruby-configuration=.
#+END_QUOTE

Loaded before Ivy so the =orderless-ivy-re-builder= symbol is bound when
Ivy first invokes it. Also sets =completion-styles= so any built-in
=completing-read= caller (e.g. =describe-variable=) picks it up too.
...
#+BEGIN_QUOTE
Annotations in the completion margin — docstrings for =M-x=, file sizes
for =find-file=, major modes for buffer switching, etc.
#+END_QUOTE
...
#+BEGIN_QUOTE
A front-end for ripgrep. Results open in a dedicated, navigable buffer,
which stays responsive on large result sets.
#+END_QUOTE
...
Rebind projectile's default search (=C-c p s s=) to =counsel-projectile-rg=
so incremental in-minibuffer search uses ripgrep instead of ag.
